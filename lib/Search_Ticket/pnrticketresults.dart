...
class PnrTicketResults extends StatefulWidget
{
  ...
  const PnrTicketResults(
      {Key? key, required this.pnr}) : super(key: key);
...
class _PnrTicketResultstState extends State<PnrTicketResults> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder(future: getTicket(widget.pnr),
          builder: (BuildContext context, AsyncSnapshot snapshot)
      {
        ...
        if(snapshot.hasData)
        {
          print(widget.pnr.toString()+"ssass");
              ...
              itemBuilder: (BuildContext context, int Index)
          {
            return Row(
              children: [
                Expanded(child: Padding(padding:const EdgeInsets.fromLTRB(8.0, 8.0, 8.0, 2),
                child: Container(
                    ...
                    children: [
                      Center(
                        child: Padding(padding:const EdgeInsets.fromLTRB(
                            8.0, 8.0, 8.0, 2),
                        child: Text(snapshot.data[Index].seatName,
                      ...
                      ElevatedButton(onPressed: (){}, child:Text("Download Ticket "))
...
Future<List<ticketmodel>> getTicket(String a) async {
  ...
  return list.map((element) => ticketmodel.fromJson(element)).where((element) => element.transactionId!.toLowerCase().toString()==a.toString().toLowerCase()).toList();
}
